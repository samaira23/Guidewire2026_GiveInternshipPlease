...
export default function DisputesPanel() {
	...
	const fetchDisputes = () => {
		fetch(`${import.meta.env.VITE_APP_BACKEND_URL || 'https://giveinternshipplease-backend.onrender.com'}/api/admin/disputes/`, {
	...
	const resolveClaimBackend = async (id, action) => {
		try {
			await fetch(`${import.meta.env.VITE_APP_BACKEND_URL || 'http://localhost:8000'}/api/admin/disputes/${id}/resolve/`, {
				method: 'POST',
				headers: {
