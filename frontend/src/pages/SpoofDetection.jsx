...
export default function SpoofDetection() {
  ...
  const verifyClaim = async () => {
    ...
    let fraudScore = 0;
    try {
      const req = await fetch('http://localhost:8000/api/crosscheck/', {
        method: 'POST',
    ...
    if (wantAQ) {
      ...
      try {
        const req = await fetch('http://localhost:8000/api/air-quality/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lat, lon })
        });
    ...
    if (wantWX) {
      ...
      try {
        const req = await fetch('http://localhost:8000/api/weather/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lat, lon })
        });
