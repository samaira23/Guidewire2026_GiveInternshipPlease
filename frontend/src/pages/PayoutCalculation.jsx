...
import { submitClaim, updateClaimStatus } from '../api';
...
export default function PayoutCalculation() {
  ...
  const hazardBonus = 120;
  const estimatedMilesBonus = 35;
  const totalPayout = basePayout + hazardBonus + estimatedMilesBonus;
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      // Jump directly to Paid/Approved to simulate the payout in the unified flow
      await fetch(`http://localhost:8000/api/claims/${newClaim.id}/`, {
        method: 'PATCH',
        headers: {
      ...
      setSuccessMsg("Claim Approved and payout initiated!");
      ...
      <div className="flex-1 p-6 bg-[#0f1117] min-h-screen text-white">
        ...
        <div className="max-w-2xl mx-auto mt-6">
          ...
          <div className="bg-[#1a1d24] border border-[#2a2f3a] p-8 rounded-2xl shadow mb-6 text-center">
            ...
            <p className="text-xs text-gray-500 mb-8 max-w-sm mx-auto">This integer value is finalized and ready to be forwarded to the Razorpay API gateway for processing.</p>
